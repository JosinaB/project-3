...
account_selected = st.radio('Are you a Artist Donator , or you are a Buyer?', account_selection, index=1)
...
#Select_account = st.selectbox("Select Artist Account", options=accounts)
#certificate_details = st.text_input("Certificate Details", value="Art Ownership Certification")




def pin_cert(name, cert_file):
...
cert_name = st.text_input("name of the item you want to certify?: ")


file = st.file_uploader("Upload Art", type=["jpeg", "pdf"])


if st.button("Award Certificate"):
    cert_ipfs_hash = pin_cert(cert_name, file)
    cert_uri = f"ipfs://{cert_ipfs_hash}"
    tx_hash = contract.functions.registerArt(account,cert_uri).transact({'from': account, 'gas': 1000000})
